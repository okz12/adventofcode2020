...
class Grid:
    ...
    def parse(string: str, w_dim: bool = False) -> Grid:
        grid = {(x, y, 0, 0) for y, line in enumerate(string.split("\n"))
                for x, char in enumerate(list(line)) if char == "#"}
        ...
        for w in range(min(w_), max(w_) + 1):
            for z in range(min(z_), max(z_)+1):
                ...
                for y in range(min(y_), max(y_)+1):
                    grid += "\n" + "".join("#" if (x, y, z, w) in self.active else "." for x in range(min(x_), max(x_)+1))
        return grid
